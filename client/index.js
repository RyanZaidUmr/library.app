...
function generateRows(books) {
  let rows = '';
  if (books.length === 0) {
    rows = `<tr><td colspan="6" class="px-6 py-4 border-b text-center">Tidak ada buku yang ditemukan</td></tr>`;
  } else {
    books.forEach((book) => {
...
async function addBook(book) {
  try {
    ...
    await fetchBooks(); // Refresh the book list after adding a book
  } catch (error) {
    console.log(error);
    console.log('Terjadi kesalahan saat menambah buku');
  }
...
async function editBook(book) {
  try {
    ...
    await fetchBooks(); // Refresh the book list after editing a book
  } catch (error) {
    console.log(error);
    console.log('Terjadi kesalahan saat mengubah buku');
  }
...
async function deleteBook(bookId) {
  try {
    ...
    await fetchBooks(); // Refresh the book list after deleting a book
  } catch (error) {
    console.log(error);
    console.log('Terjadi kesalahan saat menghapus buku');
  }
...
// Load the initial page when the app starts
loadPage();
